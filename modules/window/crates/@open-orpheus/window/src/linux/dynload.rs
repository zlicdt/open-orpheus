...
///
/// // Public accessor:
/// dynamic_fn!(pub my_public_symbol, MyFnType, "my_c_symbol");
/// ```
///
/// Each form expands to a lazily-resolved accessor that resolves the symbol on
/// first call and caches the result in a `OnceLock`.
#[macro_export]
macro_rules! dynamic_fn {
    // Optional visibility + explicit C symbol name.
    ($vis:vis $name:ident, $fn_type:ty, $symbol:literal) => {
        $vis fn $name() -> Result<$fn_type, String> {
            static RESOLVED: ::std::sync::OnceLock<Result<$fn_type, String>> =
                ::std::sync::OnceLock::new();
            RESOLVED
                .get_or_init(|| unsafe {
                    $crate::linux::dynload::resolve_symbol::<$fn_type>(
    ...
    // Optional visibility + accessor name doubles as the C symbol name.
    ($vis:vis $name:ident, $fn_type:ty) => {
        $vis fn $name() -> Result<$fn_type, String> {
            static RESOLVED: ::std::sync::OnceLock<Result<$fn_type, String>> =
                ::std::sync::OnceLock::new();
            RESOLVED

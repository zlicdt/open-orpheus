...
fn draw_menu_items(
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        let width = ui.available_width().min(MENU_WIDTH);
...
fn measure_items(items: &[MenuItem]) -> Vec2 {
    ...
    crate::util::measure_ui(App::create_context(), MENU_WIDTH, move |ui| {
        ui.style_mut().spacing.item_spacing.y = 0.0;
        draw_menu_items(ui, items_ref, |_, _| None);
    })
}
...
impl Menu {
    ...
    pub fn show(&self) {
        ...
        #[cfg(target_os = "linux")]
        if app.is_wayland() {
            smol::spawn(show_wayland_overlay(app, menu_data.content.clone(), None)).detach();
            return;
        }
        smol::spawn(Self::show_menu_with_items(
            app,
    ...
    pub fn show_at(&self, x: f64, y: f64) {
        ...
        #[cfg(target_os = "linux")]
        if app.is_wayland() {
            smol::spawn(show_wayland_overlay(
                app,
                menu_data.content.clone(),
                Some(egui::Pos2::new(x as f32, y as f32)),
            ))
            .detach();
            return;
        }
...
#[cfg(target_os = "linux")]
async fn show_wayland_overlay(app: App, items: Arc<Vec<MenuItem>>, anchor: Option<egui::Pos2>) {
    use std::sync::Mutex;
    ...
    // Stack of open levels: (items, top-left position in egui space).
    // Level 0 is the root menu; each subsequent entry is a submenu.
    let levels: Arc<Mutex<Vec<(Arc<Vec<MenuItem>>, egui::Pos2)>>> = Arc::new(Mutex::new({
        let root_pos = anchor.unwrap_or(egui::Pos2::ZERO);
        vec![(items, root_pos)]
    }));
    ...
    let render_ready = Arc::new(AtomicBool::new(anchor.is_some()));
    ...
    let (ctx, window_id) = app
        .create_egui_window(ViewportId::from_hash_of("wayland_overlay_menu"), builder, {
            ...
            move |ctx| {
                    ...
                    let area_resp = egui::Area::new(area_id)
                        ...
                        .show(ctx, |ui| {
                            egui::Frame::popup(ui.style()).show(ui, |ui| {
                                ...
                                draw_menu_items(ui, &level_items, |idx, response| {
                                    ...
                                    if response.hovered() {
                                        ...
                                        return Some(Color32::from_rgba_unmultiplied(0, 0, 0, 50));
                                    }
                                    if response.is_pointer_button_down_on() {
                                        return Some(Color32::from_rgba_unmultiplied(0, 0, 0, 75));
                                    }
                                    None
                                });
                            });
                ...
                if let Some((parent_depth, sub_pos, children)) = pending_submenu {
                    ...
                    levels_guard.push((children, sub_pos));
